// cl demo.c openvr_api.lib opengl32.lib user32.lib gdi32.lib
#include <windows.h>
#include <gl/gl.h>
#include <stdio.h>
...
void vrInit()
{
	if(!VR_IsHmdPresent() || !VR_IsRuntimeInstalled()) ExitProcess(0);
	EVRInitError error;
	VR_InitInternal(&error, EVRApplicationType_VRApplication_Scene);
	char table[128];
	struct VR_IVRSystem_FnTable *vr = (struct VR_IVRSystem_FnTable*) VR_GetGenericInterface(table, &error);
	struct VR_IVRCompositor_FnTable *vrc = (struct VR_IVRCompositor_FnTable*) VR_GetGenericInterface(table, &error);
}
